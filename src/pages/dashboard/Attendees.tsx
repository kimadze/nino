    ...
    <div className="space-y-8">
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "სულ რეგისტრაცია", value: stats.total ?? 0 },
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 sm:max-w-sm">
          ...
          <Input placeholder="ძიება სტუმრებში…" className="pl-10 rounded-full bg-card" value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} />
        </div>
        <Select value={eventFilter} onValueChange={v => { setEventFilter(v); setPage(0); }}>
          <SelectTrigger className="w-full sm:w-56 rounded-full bg-card">
            <SelectValue placeholder="გაფილტრე ღონისძიებით" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">ყველა ღონისძიება</SelectItem>
      ...
      {isLoading ? (
              ...
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("name")}>
                    <span className="flex items-center text-xs font-semibold uppercase tracking-wider text-muted-foreground">სახელი <SortIcon col="name" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none hidden sm:table-cell" onClick={() => handleSort("email")}>
                    <span className="flex items-center text-xs font-semibold uppercase tracking-wider text-muted-foreground">ელფოსტა <SortIcon col="email" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none hidden md:table-cell" onClick={() => handleSort("event")}>
                    <span className="flex items-center text-xs font-semibold uppercase tracking-wider text-muted-foreground">ღონისძიება <SortIcon col="event" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("status")}>
                    <span className="flex items-center text-xs font-semibold uppercase tracking-wider text-muted-foreground">სტატუსი <SortIcon col="status" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none hidden lg:table-cell" onClick={() => handleSort("date")}>
                    <span className="flex items-center text-xs font-semibold uppercase tracking-wider text-muted-foreground">თარიღი <SortIcon col="date" /></span>
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
        ...
        <div className="text-center py-20">
          <p className="text-muted-foreground">{search || eventFilter !== "all" ? "შესაბამისი სტუმრები ვერ მოიძებნა." : "ჯერ რეგისტრაციები არ არის."}</p>
        </div>
      )}
      ...
      <Dialog open={!!selectedRegistration} onOpenChange={(open) => !open && setSelectedRegistration(null)}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="font-display">სტუმრის დეტალები</DialogTitle>
          </DialogHeader>
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="border-t border-border pt-3 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground font-medium">ღონისძიება</span>
                    <span className="text-sm">{selectedRegistration.events?.name || "—"}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground font-medium">სტატუსი</span>
                    <Badge className={`${statusStyle[selectedRegistration.status] || ""} capitalize text-xs`}>
                      {selectedRegistration.status.replace("_", " ")}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground font-medium">დარეგისტრირდა</span>
