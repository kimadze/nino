...
const Analytics = () => {
  ...
  const statCards = [
    { label: "სულ რეგისტრაცია", value: stats?.total ?? 0, icon: Users },
    { label: "აქტიური ღონისძიებები", value: stats?.activeEvents ?? 0, icon: CalendarDays },
    { label: "საშუალო ღონისძიებაზე", value: stats?.events?.length ? Math.round((stats?.total ?? 0) / stats.events.length) : 0, icon: TrendingUp },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-card rounded-2xl p-5 sm:p-7">
          ...
          {perEventData.length > 0 ? (
            ...
            <p className="text-muted-foreground text-center py-12 text-sm">ჯერ მონაცემები არ არის.</p>
          )}
        </div>
