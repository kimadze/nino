...
const SettingsPage = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.error(err.message || "პროფილის განახლება ვერ მოხერხდა");
    }
  };
  ...
  const handleInvite = async () => {
    ...
    try {
      toast.success(`მოწვევა გაიგზავნა ${inviteEmail}-ზე`);
      setInviteEmail("");
    } catch (err: any) {
      toast.error(err.message || "მოწვევის გაგზავნა ვერ მოხერხდა");
    } finally {
    ...
    <div className="space-y-8 max-w-3xl mx-auto">
      ...
      <Tabs defaultValue="profile">
        <TabsList className="bg-muted rounded-full p-1">
          <TabsTrigger value="profile" className="rounded-full data-[state=active]:bg-card data-[state=active]:shadow-sm">პროფილი</TabsTrigger>
          <TabsTrigger value="appearance" className="rounded-full data-[state=active]:bg-card data-[state=active]:shadow-sm">თემა</TabsTrigger>
          <TabsTrigger value="team" className="rounded-full data-[state=active]:bg-card data-[state=active]:shadow-sm">გუნდი</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="profile" className="mt-6">
          <div className="bg-card rounded-2xl p-6 sm:p-8 space-y-5">
            ...
            <div className="space-y-2">
              <Label>კომპანიის აღწერა</Label>
              <Textarea
                value={companyDescription}
                onChange={e => setCompanyDescription(e.target.value)}
                placeholder="მოკლე აღწერა შენი კომპანიის ან ორგანიზაციის შესახებ…"
                rows={3}
            ...
            {companySlug && (
              <div className="space-y-2">
                <Label>საჯარო კომპანიის გვერდი</Label>
            ...
            <div className="space-y-3">
              ...
              <Button type="button" variant="outline" size="sm" onClick={addSocialLink} className="rounded-full">
                <Plus className="w-4 h-4 mr-1" /> სოც. ქსელის დამატება
              </Button>
            </div>
        ...
        <TabsContent value="appearance" className="mt-6">
          <div className="bg-card rounded-2xl p-6 sm:p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {([
                { value: "light", label: "ნათელი", icon: Sun, desc: "სუფთა და ნათელი ინტერფეისი" },
                { value: "dark", label: "მუქი", icon: Moon, desc: "თვალისთვის მოსახერხებელი" },
                { value: "system", label: "სისტემური", icon: Monitor, desc: "მოწყობილობის პარამეტრების მიხედვით" },
              ] as const).map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => { setTheme(opt.value); toast.success(`თემა: ${opt.label}`); }}
        ...
        <TabsContent value="team" className="mt-6">
          <div className="bg-card rounded-2xl p-6 sm:p-8 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              მოიწვიე სხვები, ვინც დაგეხმარება ღონისძიებების ორგანიზებაში.
            </p>
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              >
                {inviteSending ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Send className="w-4 h-4 mr-1" /> გაგზავნა</>}
              </Button>
            </div>
          </div>
        </TabsContent>
