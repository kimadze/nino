...
export default function EventDetailHeader({ event, onStatusChange }: Props) {
  ...
  const handleCopyLink = () => {
    ...
    toast.success("რეგისტრაციის ბმული დაკოპირდა!");
  };
      ...
      <div className="flex items-start gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground">
            {event.event_date ? new Date(event.event_date).toLocaleDateString() : "თარიღი არ არის"} · {event.event_type || "ღონისძიება"}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="hidden sm:flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="h-8 text-xs rounded-full" onClick={handleCopyLink}>
            <Copy className="w-3 h-3 mr-1" /> ბმულის კოპირება
          </Button>
          <Button variant="outline" size="sm" className="h-8 text-xs rounded-full" asChild>
            <a href={`/register/${event.slug}`} target="_blank" rel="noopener noreferrer">
              <ExternalLink className="w-3 h-3 mr-1" /> ნახვა
            </a>
          </Button>
        </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={handleCopyLink}>
                <Copy className="w-3.5 h-3.5 mr-2" /> ბმულის კოპირება
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <a href={`/register/${event.slug}`} target="_blank" rel="noopener noreferrer">
                  <ExternalLink className="w-3.5 h-3.5 mr-2" /> ნახვა
                </a>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
