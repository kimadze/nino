...
const navItems = [
  { title: "ღონისძიებები", url: "/dashboard/events", icon: CalendarDays },
  { title: "სტუმრები", url: "/dashboard/attendees", icon: Users },
  { title: "ანალიტიკა", url: "/dashboard/analytics", icon: BarChart3 },
  { title: "ინტეგრაციები", url: "/dashboard/integrations", icon: Puzzle },
  { title: "პარამეტრები", url: "/dashboard/settings", icon: Settings },
];
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const allItems = [
    ...
    ...(profile?.company_slug
      ? [{ title: "კომპანია", url: `/company/${profile.company_slug}`, icon: Eye }]
      : []),
  ];
